...
func (s *Notifier) Enqueue(userId int64, userCommentsCount int64) {
	s.mutex.Lock()

	s.queueMap[fmt.Sprintf("%v", userId)] = eventData{
		UserId: userId,
		Count:  userCommentsCount,
	}
...
func (s *Notifier) Flush() []error {
	apmTransaction := apm_helper.StartNewApmTransaction("user comments counter flush", "publisher", nil, nil)
	s.mutex.Lock()

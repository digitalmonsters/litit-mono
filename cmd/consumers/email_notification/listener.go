...
func InitListener(appCtx context.Context, configuration boilerplate.KafkaListenerConfiguration,
	notificationSender sender.ISender, emailLinks configs.EmailLinks) kafka_listener.IKafkaListener {
	return kafka_listener.NewSingleListener(configuration, kafka_listener.NewCommand("email notifications",
		func(executionData kafka_listener.ExecutionData, request ...kafka.Message) []kafka.Message {
			...
			result, err := process(*mapped, executionData.Context, notificationSender, emailLinks)

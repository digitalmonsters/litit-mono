...
var counterService *sharesCounterService
...
func TestProcessUpdateCounters(t *testing.T) {
	...
	testMap := make(map[int64]*sharesCount)
	testMap[song.Id] =
		&sharesCount{
			ContentEvent: eventsourcing.ContentEvent{
				Id:          song.Id,
				SharesCount: song.UserId + 50,
	...
	for _, c := range songs {
		a.Equal(int(c.UserId+50), c.Shares)
	}
}
